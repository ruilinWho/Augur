...
- **状态：** 已接受（信源 + EDGAR）；X 官方号**待作者定夺**（见下）
- **日期：** 2026-06-02
- **决策者：** 作者 + Claude

> 作者原话（要点）：①「知」里的美股信源还不够好，**调研怎么找到最好的美股信源**；②每天找更多信息（**10x**）——「更多的信息代表更多的机会」，但**一定要用顶级的、一手的信源，这是整个系统的价值、生死攸关所在**；③考虑接入 X(Twitter) 官方号（Anthropic/NVIDIA/OpenAI…）；④对关注的股票，**一条龙接入所有信源**（官方网页、官方 X…），「基于 LLM 的能力可以做到吧？不要省 Token」。

## 背景

- M3 起步版 41 个泛源里，行情/宏观只有 CNBC + 华尔街见闻两条二手；前沿方向虽好，但**缺一手**（央行原文、官方数据、公司新闻室、监管文件、个股财报）。
- 护栏：只研究不交易、不构成建议、**暴露不确定性**、**本地优先**（数据只发往作者配置的厂商）、尊重限流（§1/§5/§11）。
...
`feeds.yaml` 从 41 扩到 **~90**，硬验证 **90/93 可抓**（个别 Cloudflare 源数据中心 IP 被挡、作者住宅 IP 可达，已注明并保留——ingest 容忍单源失败、不会因偶发 403 删源）。新增主力：
...
作者问「基于 LLM 一条龙接入个股所有信源，做得到吧」。**做得到，但一手文件的「获取」要的是「不编造」**——因此用**结构化官方 API**，不用 LLM 抓网页：
...
- SEC 公平获取：可配置 UA（`config.sec_user_agent`，默认非 PII；作者可在 `.env` 配 `AUGUR_SEC_USER_AGENT="名字 邮箱"` 最稳妥）、进程内 **≤10 req/s** 节流、**6h 缓存**、失败降级空表。
...
### 3. X(Twitter) 官方号 —— 选项与权衡（**待作者定夺**）

调研结论（2026-06）：作者关心的账号（OpenAI/NVIDIA/@sama/@elonmusk…）**只活在 X**；Bluesky/Mastodon 上同名号要么不存在要么 0 帖（连 LeCun 都回 X 了）。而 X 没有免费、稳定、隐私友好的官方 RSS：

| 路径 | 现状（2026） | 裁决 |
|---|---|---|
| 官方 X API | 改按量计费、Basic 对新开发者关闭、需开发者账号 + OAuth | 官僚、政策脆，不作主力 |
| Nitter | 2024 已停摆，公共实例靠激进限流苟活，不适合无人值守 | 弃 |
| RSSHub（自托管） | Twitter 路由需塞自己的 X 登录 cookie（封号风险）+ 维护成本高 | 仅当作者要「零第三方」且接受 cookie 风险 |
| **TwitterAPI.io（商业桥）** | `GET /twitter/user/last_tweets?userName=`，X-API-Key、**无需作者 X 账号**，~$0.15/1k 推、号称 99.99% | **推荐主力** |
| SocialData | 同模式，~$0.20/1k | 备选/降级 |
| Bluesky/Mastodon 原生 RSS | 免费、最私密，但前沿账号几乎没人在用 | 顺带订阅，不作主力 |

**隐私权衡（§5 关键）**：商业桥只需公开用户名 + API key，**不碰作者 X 账号/任何 PII**，符合「经桥抓公开数据可接受」；残留暴露＝桥会知道你**轮询了哪些账号**。若不可接受，唯一零第三方路径是自托管 RSSHub + 自己 cookie（封号风险更糟）。

**为何不擅自接**：X 接入要花钱（第三方/官方）、要作者去办 key、且有上述隐私权衡——触及 §5/§11，**必须作者拍板**。故先把可换桥的 `XBridgeAdapter` 接口留好（凭 key 启用），方案与权衡呈给作者选。
