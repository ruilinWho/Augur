...
- [ ] **按钮对齐**：按钮不是装饰块。新增按钮前先确定操作列/操作行；同类命令按钮固定宽度、固定位置，对齐到同一视觉网格；凭证/文档/官网这类信息入口优先做文本链接。
...
- 按钮必须成组对齐：测试/保存/添加/删除等命令按钮要进入稳定的右侧操作列或统一操作行；不要在内容说明区散落多个小按钮。多个并列按钮必须等高、等宽或共享明确的栅格。
- 文档、凭证、官网入口优先是文本链接；只有它们是当前主要动作时才可升格为按钮。
