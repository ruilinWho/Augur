...
- 顶栏下方两栏：**上下文面板**（左，可拖宽）+ **主舞台**（右，占满剩余宽度）。面板随 Tab 变：看/研 显示「**市场过滤器**（分段控件 全部/美/港/A/韩）+ **两级自选分区 Miller 列**」；知 显示资讯/个股的 Miller 导航；记显示笔记列表；设置显示外观/模型/数据信源/自动任务等分类。**市场是正交过滤器，不是板块第三层**。
