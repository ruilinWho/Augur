...
- 雪球：已接轻量内容探活；如果返回 WAF/网页壳，说明当前填入的 Cookie 不足以访问讨论内容 JSON。若已有 `xq_a_token` + `u` 但仍失败，优先提示缺少 `acw_sc__v2` 等风控 Cookie，并要求从浏览器 Network 的真实 `xueqiu.com` 请求复制完整 `Cookie` header，不要从 Application/Cookies 逐项拼。
