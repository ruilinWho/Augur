...
>
> **状态（2026-06-11 更新）**：twtapi **已退役为全局推特源**——全局推特唯一走 TikHub（前缀 `X·`）。
> twtapi 适配器（`news/twtapi.py`）**仅每股专属 X 信源仍用**（按账号拉 `official_x`/`influencer_x`，
> 失败自动回退 `tikhub.user_tweets`）。下面的端点/额度/诊断坑仍适用于这条每股路径。
