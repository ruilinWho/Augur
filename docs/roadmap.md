...
- ✅ **多信源 lane 扩展（新闻 / 推特 / Reddit / 雪球 / 小红书）**：前端「资讯」第三层从固定新闻/推特改为多 source lane；已接入 lane 共用时间线/要点/市场/仅自选过滤和标股 chip。**Reddit 已真实接入**：`reddit.py` 走 public JSON（配置 subreddit，source=`Reddit·r/<sub>`），并入 ingest→翻译→relevance→linker→stock_tag 流水线；设置页可配 subreddit 且可测试。**雪球**：已登记 token、关注用户、个股/关键词配置与 UI lane，`xueqiu.py` 已接入轻量内容探活但未并入 ingest；完整 Cookie header 不足时会明确报 WAF/网页壳，若已有登录 token 但缺 `acw_sc__v2` 会提示从 Network 复制真实请求 Cookie。**小红书**：仍只登记配置与 UI lane，不伪装抓取。
