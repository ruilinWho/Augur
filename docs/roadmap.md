...
- ✅ **「知」二层噪音过滤升级**：在 `filter.py` 规则之上加 `relevance.py`（cheap 小模型批量判投资相关性，丢娱乐/消费/生活，feed/日报/机会取 `relevance≠2`）；实测 240 判/56 弃。
- ✅ **个股相关新闻接 API**：`ticker_news.py`（yfinance `.news` 逐 ticker，四市场）∪ 聚合流按名匹配。
