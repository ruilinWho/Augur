...
**M1 / M1.5 / M1.6 / M2 已完成 ✅，M3「知」推进中，新增第 4 支柱「记」✅。** 四根支柱齐活：**看**（四市场 K 线 + 基本面 + 财报趋势 + 判断日记 marker + 52 周位置）、**研**（单股深度研究编排：确定性数据 → 长上下文模型流式带引用报告）、**知**（~90 源摄取 + 分类 + 翻译 + 趋势日报 + 投资机会 + 个股叙事 + 二层噪音过滤）、**记**（与个股无关的自由长文笔记）。下一步：研的深度增强（实时网络/Deep Research、按分区批量）、知的续做（X 接入、按分区聚合、信源健康度）。
