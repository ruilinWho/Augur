...
- **Counter-evidence radar (雷达 lane)** — per-stock theses (bull/bear/watch + falsifiable invalidation conditions, AI-draftable) monitored against each day's linked news: a ⚡refute when a condition is materializing, a support signal when it is clearly not, each with cited evidence. Conditions are author-owned; alerts are deterministically validated and roll over a 7-day window, rescanned by `generate_all`. A ⚡反证 badge surfaces inline on the 分区 movers. See [counter-evidence-radar](memory/counter-evidence-radar.md).
