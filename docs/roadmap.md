...
- ✅ **新闻「要点」（去重聚类 + 按投资重要性排序）**（主人反馈「新闻重复、信息熵低」）：`cluster.py` 逻辑入 `service.generate_clusters(date, theme)`——把当天（主题内）经筛选的新闻喂 summarize 模型，**按事件去重合并成簇** + 每簇给**合并中文标题 / 重要性(high/med/low) / 一句为何 / 成员[n]**，按重要性排序、纯噪音丢弃；落库 `news_clusters`（date,theme 唯一，重生成覆盖）。`GET/POST /news/clusters`。前端**「新闻」视图加「时间线 / 要点」切换**：要点=重要性排序的簇卡（重要/一般/次要徽标 + 合并标题 + 一句价值 + 展开多源链接，次要默认折叠）。提示词 `news_clusters.md`。真机：200 条→102 簇、high 15/med 75/low 12、Marvell 簇合并 7 源。
