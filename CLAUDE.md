...
- **下一步：** M3 续——机会接地阈值真机抽查调优、日报/机会按**自选分区**聚合（板块相关）、信源健康度、机会去重（双重上市）；M2 续——`research/` 深度研究编排（多轮：行情+基本面+新闻+网络 → 综合 → 引用）。
