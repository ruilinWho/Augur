...
- **深夜产品修正（新）：** 「资讯」source lane 的 cluster scope 按真实 lane 分离，避免 Reddit/雪球/小红书读到新闻/推特旧要点；雪球/小红书 lane 明确待接入且设置页支持**关注用户 + 个股/关键词**；cluster 标题/理由清洗 `[47]` 这类裸编号；「决策」显示当日独立信源数；「看·相关资讯」改为 AI 摘要卡（`/news/for/brief`），不再铺直接新闻列表；「记」去冗余标题/空态文案；设置·模型连接卡默认折叠；数据/信源名称保持唯一官方名，Bloomberg 频道成为可配置项。
