...
- **看：** 四市场 K 线、报价头部、52 周位置、基本面、财报趋势、判断日记 marker；**个股资讯合并为单一「相关资讯」**板块（现状一句话 + 要点 + 时间线 + 社媒热度，一眼看全；要点说人话且带可点来源 `CitedPoint`）。头部有**一键复制 Prompt**（共享 `CopyPromptButton`，看/研共用）。详见 [stock-info-and-social-lanes](docs/memory/stock-info-and-social-lanes.md)、[view-stock-info-hierarchy](docs/memory/view-stock-info-hierarchy.md)。
...
- **知：** `feed` lane 聚合 RSS/API/推特/Reddit/TikHub 等全局信源；`ticker` lane 服务自选股定向新闻和每股专属信源。刷新链路为摄取 → 翻译 → relevance → 确定性挂钩 → LLM 标股 → grounding。日报、要事、机会、决策页、个股叙事和某日快照都围绕可追溯引用与反证条件。**推特唯一源＝TikHub**（前缀 `X·`，twtapi 桥已退役、仅每股专属信源仍用）。**社媒 lane（推特/小红书/Threads/Reddit）不套用新闻的严格 relevance、按抓取日归桶**（否则常年空，见 [stock-info-and-social-lanes](docs/memory/stock-info-and-social-lanes.md)）。**总结/决策经 `social_pulse` 聚合各社媒 lane 已生成要点接入所有信源**（零成本只读）。**微信源已删除**（TikHub `wechat_mp/web/*` 整组服务端长期 400，非我方问题，见 [tikhub-source-quirks](docs/memory/tikhub-source-quirks.md)）；cluster scope 对中文前缀用哈希，避免 `小红书·` 等全中文前缀塌缩成同一 scope 互相覆盖。**每股专属信源** kind 支持 X（twtapi，失败回退 TikHub）、**小红书/Threads 关键词（TikHub）**、Reddit、RSS/Atom（见 [per-stock-source-tracking](docs/memory/per-stock-source-tracking.md)）。**Reddit 直连 reddit.com 公共 JSON 已被按 IP 封 403**（`/r/*/new.json`、`about.json`、`subreddits/search.json` 全封，直连 lane 死）——每股 Reddit 改走 **TikHub 关键词搜该股**，`stock_sources.ensure_auto_reddit` 在首次刷新时**自动接入**该股 Reddit 源；全局 Reddit lane 靠 TikHub 兜底（见 [api-share-discovery-signals-reddit-block](docs/memory/api-share-discovery-signals-reddit-block.md)）。**设置·模型页「配置分享」**：一键导出/导入全部 API key（LLM 连接+信源 secret）JSON，明文、分享给 contributor 快速配置。
- **寻：** `discovery/` 域聚合 `news_item_symbols` 里 `matched_by='llm'` 的**自选外**挂钩，跨天累计提及次数/出现天数/证据，经 `search` top-2 做自选别名归并（跨语言/股份类/双重上市），落 `discovery_candidates`（状态机 new/dismissed/promoted）。候选带主导**主题**；前端「关注中/已忽略」切换 + **主题级 & 市场级屏蔽偏好**（不想看的整类/整个市场如韩股不再出现，可恢复）。**重算时对信号最强的前 N 个（`_JUDGE_MAX`）批量过 cheap-LLM：判「是否值得纳入投资关注」（筛掉蹭热点/八卦）+ 给一句话理由（`reason` 列展示在候选卡上）**；理由按 symbol+提及数缓存、小幅波动复用（LLM 慢，避免每次重算全量重判）。证据 URL+近重标题双重去重，条数可配。**近月大涨且放量的候选标「异动」pill**（`/discovery/signals` 懒加载、与列表解耦，`market.momentum` 缓存优先）。见 [ADR-0015](docs/decisions/0015-discovery-pillar.md)。
