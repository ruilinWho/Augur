...
- **知：** `feed` lane 聚合 RSS/API/推特/Reddit/TikHub 等全局信源；`ticker` lane 服务自选股定向新闻和每股专属信源。刷新链路为摄取 → 翻译 → relevance → 确定性挂钩 → LLM 标股 → grounding。日报、要事、机会、决策页、个股叙事和某日快照都围绕可追溯引用与反证条件。**推特唯一源＝TikHub**（前缀 `X·`，twtapi 桥已退役、仅每股专属信源仍用）。**社媒 lane（推特/小红书/Threads/Reddit/微信）不套用新闻的严格 relevance、按抓取日归桶**（否则常年空，见 [stock-info-and-social-lanes](docs/memory/stock-info-and-social-lanes.md)）。**每股专属信源** kind 支持 X（twtapi，失败回退 TikHub）、**小红书/Threads 关键词（TikHub）**、Reddit、RSS/Atom（见 [per-stock-source-tracking](docs/memory/per-stock-source-tracking.md)）。
